'''This program is designed as a budgeting tool for the user. It gets info on how much money they can afford to spend freely 
    per day on their current budget, and suggestions on how they could alter their budget to meet their daily "allowance"
    goal. The program inputs user data on income and expenses, and uses these in its budgetary calculations.

    Authors: William Micknick, Iliya Polischuk, Ori Amiel
'''
...
class BudgetCalculator:
    '''This class takes in data regarding income and expenses, and transforms this data into user metrics
        ...
        Attributes:
            ...
            savings(float): amount that goes to savings per paycheck
            goal(int): the daily allowance the user would like to have
    '''


    # python3 budget_semifinal.py
    ...
    def expense_proportions(self):
       ...
       # previous_proportion = 0
        ...
        greatest_expense_num = 0
        greatest_expense_str = ""
        ...
        for i in self.expenses: # For every expense, get the proportion of income it represents, and add it to the dict
            ...
            if proportion_of_income > greatest_expense_num:
                greatest_expense_num = proportion_of_income
                greatest_expense_str = f"{i}"
            else:
                pass
        ...
        if proportion_total > 100: # If all of the expenses are more than their total income, they are spending too much already and have no budget.
            ...
            free_money_proportion = 0

        return expense_pie_chart, greatest_expense_str, float(free_money_proportion)
    ...
    def daily_budget(self):
        ...
        if self.over_budget: 
            ...
            daily_allowance = (self.income - self.total_expenses) / 30
            return float(daily_allowance)
            


    def allowance_goal_difference(self, budget_goal, current_allowance):
        ...
        '''
        if type(current_allowance) == str:
            budget_deficit = budget_goal
        else: 
            budget_deficit = (budget_goal - current_allowance)
       
        return float(budget_deficit)
    ...
    def suggest_actions(self, deficit_to_goal, greatest_expense, hourly_wage):
        ...
        revised_budget_plan = dict(self.expenses) # New dictionary copy that will hold a budget which meets the goal
        ...
        return int(additional_hours_needed), revised_budget_plan


    def display_summary(self, expenses, biggest_expense, free_money_proportion, allowance, gap_to_goal, extra_hours, spending_plan, goal):
        '''Displays information to the user relating to the program's calulations and conclusions.
            ...
            Args:
                ...
                surplus(bool): is the user already over budget? if so, returns true
        ...
        gap_to_goal_trim = f"{gap_to_goal:.2f}"
        extra_hours_trim = f"{extra_hours:.2f}"
        expenses_trim = f"{expenses[biggest_expense]}"

        #Expenses
        print(f"Here is an overview of your current spending: {self.expenses}\n")

        #Daily allowance: are they over budget already?
        if self.over_budget == False: 
            print(f"Your current daily allowance is {allowance_trim}, which is {free_money_trim} percent of your income.")
        else:
            print("Your current expenses are more than your current income, so you have no daily allowance.\n")

        #Biggest expense
        print(f"Your biggest expense per month is {biggest_expense}, which uses {expenses_trim} percent of your income.\n")

        #What is their gap to the goal?
        if surplus:
            print(f"You have a surplus of ${gap_to_goal_abs_trim} in your daily budget.\n")
        else:
            print(f"The difference between your current unspent income and your goal is ${gap_to_goal_trim}. Let's look at how we can reduce this.\n")

        #Do they need to work any extra hours?
        if extra_hours < 0.01:
            print("You are already working enough hours to meet your goal.\n")
        else:
            print(f"To earn ${gap_to_goal_trim} more per month, you would need to work {extra_hours_trim} more hours each week.\n")

        #Do they need a budget plan revision? If so, what is it?
        if surplus:
                print("You are budgeting well and living within your means. It would be smart to add your daily budget surplus to your monthly savings.\n")
        else:
...
def main():
    ...
    rent = float(input("Enter how much you pay for rent in $: "))
    car = float(input("Enter how much you spend on your car in $: "))
    gas = float(input("Enter how much you pay for gas in $: "))
    food = float(input("Enter how much you spend on food in $: "))
    subscriptions = float(input("Enter how much you spend on subscriptions in $: "))
    savings = float(input("Enter how much goes towards savings from each paycheck in $: "))
    ...
    budget_deficit = budget.allowance_goal_difference(goal, daily_allowance)
    extra_hours, spending_reduction_plan = budget.suggest_actions(budget_deficit, greatest_expense, hourly_wage)
    budget.display_summary(expense_pie_chart, greatest_expense, free_money_proportion, daily_allowance, budget_deficit, extra_hours, spending_reduction_plan, goal)
